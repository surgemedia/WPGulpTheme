...
_gmtool_sysprop_complete() {
    local SYSPROPS="PRODUCT MODEL RELEASE MANUFACTURER BRAND BOARD DISPLAY DEVICE"
...
_gmtool_admin_create_edit_() {
    ...
    TEMPLATE_VALUE) #Do we really want that ?
        ...
        ;;
    OPTION)
        COMPREPLY=( $(compgen -W "--density --width --height --nbcpu --ram --navbar --virtualkeyboard --network-mode --bridged-if --sysprop" -- "$cur") )
        return
        ;;
    esac
}
    ...
    COMMANDS)
        if [[ $i -eq $COMP_CWORD ]]; then
            ...
            logcatdump|install|flash)
                ...
                return
                ;;
            logcatclear|adbdisconnect|adbconnect)
                return
                ;;
            push)
